...
from keyboards import (
    ...
    get_withdrawal_keyboard, get_exchange_approve_keyboard,  # добавлена клавиатура для обмена
...
# ========== ОБМЕН ВАЛЮТ (исправлено: real->virtual через заявку) ==========
...
async def process_exchange_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if exchange_type == 'real_to_virtual':
            ...
            # Создаём заявку на обмен, списываем реальные звёзды
            ...
            # Списание реальных звёзд (резервирование)
...
# Эти функции уже есть в admin.py, но продублируем здесь для надёжности
# (если они уже есть в admin.py, можно их не добавлять, но для целостности файла оставим)
...
async def approve_exchange(callback: types.CallbackQuery, callback_data: ExchangeCallback):
    """Одобрение заявки на обмен."""
    exchange_id = callback_data.exchange_id
    exchange_type = callback_data.exchange_type  # real_to_virtual или virtual_to_real
    if not has_access(callback.from_user.id, 'admin'):
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    ...
    if from_cur == 'real' and to_cur == 'virtual':
        # При обмене real->virtual: начисляем виртуальные
        ...
        # При обмене virtual->real: уже списано, просто меняем статус
        success_text = f"✅ Ваша заявка на обмен #{exchange_id} одобрена!\n" \
...
async def reject_exchange(callback: types.CallbackQuery, callback_data: ExchangeCallback):
    ...
    if result:
        ...
        # Возвращаем средства в зависимости от направления
        if from_cur == 'real':
            # Возвращаем реальные
