...
async def process_ticket_message(message: types.Message, state: FSMContext):
    ...
    # Добавляем первое сообщение с медиа
    ...
    try:
        ...
        # Отправляем сообщение в группу (с медиа, если есть)
        if media_type == 'photo':
...
async def rate_ticket_callback(callback: types.CallbackQuery, callback_data: TicketCallback):
    ...
    agent_id = ticket[8] if len(ticket) > 8 else None
    if not agent_id:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''
...
async def group_message_handler(message: types.Message):
    """Обрабатывает сообщения из топиков группы."""
...
async def handle_group_message(message: types.Message):
    ...
    # Сообщение от сотрудника – сохраняем в БД
...
# ========== РЕАЛИЗАЦИЯ КНОПОК МЕНЮ В ГРУППЕ ==========
