...
def init_db():
    ...
    # --- СУЩЕСТВУЮЩИЕ ТАБЛИЦЫ (ИЗ СТАРОЙ БАЗЫ) ---
    cursor.execute('''
...
# ========== СТАРЫЕ ФУНКЦИИ (ИЗ ВАШЕГО ИСХОДНОГО database.py) ==========
# ПОЛНОСТЬЮ СОХРАНЕНЫ БЕЗ ИЗМЕНЕНИЙ
...
def add_ticket_message(ticket_id: int, user_id: int, message: str, is_from_support: bool = False):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "INSERT INTO ticket_messages (ticket_id, user_id, message, is_from_support) VALUES (?, ?, ?, ?)",
            (ticket_id, user_id, message, 1 if is_from_support else 0)
        )
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def create_withdrawal(user_id: int, amount: int, screenshot_path: str, recipient_username: str):
    ...
    try:
        ...
        payout_amount = int(amount * (1 - WITHDRAW_COMMISSION))
...
def create_exchange(user_id: int, from_currency: str, to_currency: str, amount: int, recipient_username: str = None):
    ...
    try:
        if from_currency == 'real' and to_currency == 'virtual':
            converted = int(amount * REAL_TO_VIRTUAL_RATE * (1 - EXCHANGE_COMMISSION))
            commission = int(amount * REAL_TO_VIRTUAL_RATE * EXCHANGE_COMMISSION)
            status = 'completed'
        else:
            converted = int(amount * VIRTUAL_TO_REAL_RATE * (1 - VIRTUAL_TO_REAL_COMMISSION))
            commission = int(amount * VIRTUAL_TO_REAL_RATE * VIRTUAL_TO_REAL_COMMISSION)
            status = 'pending'
...
def create_referral_reward(referrer_id: int, referred_id: int, purchase_id: int, amount: int):
    ...
    try:
        ...
        if cursor.fetchone():
            conn.close()
            return False
        reward_amount = int(amount * REFERRAL_PERCENT / 100)
        cursor.execute(
...
def create_stars_payment(user_id: int, amount: int, payload: str) -> int:
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "INSERT INTO stars_payments (user_id, amount, payload) VALUES (?, ?, ?)",
        ...
        conn.rollback()
        logger.error(f"Ошибка создания stars_payment: {e}")
        return None
    finally:
        conn.close()

def get_stars_payment_by_charge_id(charge_id: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "SELECT * FROM stars_payments WHERE charge_id = ?",
            (charge_id,)
        )
        return cursor.fetchone()
    except Exception as e:
        logger.error(f"Ошибка получения stars_payment: {e}")
        return None
    finally:
        conn.close()

def complete_stars_payment(charge_id: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        cursor.execute(
            "UPDATE stars_payments SET status = 'completed', completed_at = CURRENT_TIMESTAMP WHERE charge_id = ?",
            (charge_id,)
        )
        conn.commit()
        return True
        ...
        conn.rollback()
        logger.error(f"Ошибка завершения stars_payment: {e}")
        return False
    finally:
        conn.close()

# ========== НОВЫЕ ФУНКЦИИ (ДОПОЛНИТЕЛЬНЫЙ ФУНКЦИОНАЛ) ==========

# --- Кэширование баланса ---
def get_cached_balance(user_id: int, currency: str = 'virtual') -> int:
    cache_key = f"balance:{user_id}:{currency}"
    ...
    if not user:
        return 0
    balance = user[5] if currency == 'virtual' else user[4]
    cache_set(cache_key, balance, ttl=60)
    return balance

def invalidate_balance_cache(user_id: int):
    cache_delete(f"balance:{user_id}:virtual")
    cache_delete(f"balance:{user_id}:real")

# --- Кэширование топа ---
def get_cached_top_buyers(limit: int = 10):
    cache_key = "top_buyers"
    cached = cache_get(cache_key)
    if cached is not None:
        return cached
    result = get_top_buyers_no_admins(limit)
...
def is_duplicate_action(action_id: str, ttl: int = 5) -> bool:
    cache_key = f"action:{action_id}"
    if cache_get(cache_key) is not None:
        return True
    cache_set(cache_key, "1", ttl=ttl)
    return False
...
# --- Приоритеты и назначение тикетов ---
...
# --- Управление ссылками-скидками (админка) ---
...
def round_price(price: float) -> float:
    if is_rounding_enabled():
        return round(price)
    return price

# --- Проверка заморозки для middleware ---
def check_freeze_and_get_info(user_id: int) -> dict:
    if is_user_frozen(user_id):
        info = get_freeze_info(user_id)
        return {
            "frozen": True,
            "reason": info[0] if info else "Не указана",
            "date": info[1] if info else None
        }
    return {"frozen": False}
...
# --- Технические работы (обновлённые функции с таймером) ---
def set_maintenance_mode(enabled: bool, reason: str = None, duration_minutes: int = None):
    ...
    Если enabled=True и duration_minutes задано, сохраняет время окончания работ.
    """
    ...
    if enabled and duration_minutes:
        ...
        # Если выключаем или не задана длительность – удаляем until
...
def get_maintenance_info() -> dict:
    """Возвращает информацию о текущем режиме ТО: причина и оставшееся время."""
...
def get_user_feedback(user_id: int, limit: int = 10):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
# --- Реферальные уровни (из настроек) ---
...
# --- Экономика (получение курсов из БД) ---
def get_star_rate():
...
# --- Статистика по пользователям (активные/неактивные) ---
