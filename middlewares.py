...
from helpers import has_access, format_datetime  # <-- ИЗМЕНЕНО: вместо from utils
...
check_maintenance_middleware = CheckMaintenanceMiddleware()            if isinstance(event, Message):
                ban_text = "🚫 ВЫ ЗАБАНЕНЫ!\n\n"
                ban_text += f"Причина: {reason}\n"
                if banned_until:
                    try:
